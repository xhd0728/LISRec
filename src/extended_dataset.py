...
class Music4AllOnion(BaseDataset):
    def __init__(
        ...
        if self.interaction_type == "counts":
            ...
            self.output_inter_file = os.path.join(
                self.output_path, self.dataset_name + "_counts.inter"
            )
        elif self.interaction_type == "timestamp":
            self.inter_file = os.path.join(
                self.input_path, "userid_trackid_timestamp.tsv"
            )
            # selected feature fields
            self.inter_fields = {
                0: "user_id:token",
            ...
            self.output_inter_file = os.path.join(
                self.output_path, self.dataset_name + "_timestamp.inter"
            )
        ...
        if feature_name != "none":
            ...
            self.output_item_file = os.path.join(
                self.output_path, self.dataset_name + "_" + feature_name + ".item"
            )
            if self.float_seq:
                item_fields_values = ["track_id:token", f"{feature_name}:float_seq"]
                self.item_fields = {
                    key: value for key, value in enumerate(item_fields_values)
                }
                ...
                with open(self.item_file, "r") as f:
                    ...
                    item_fields_values = [
                        column_name + ":float" for column_name in feature_header
                    ]
    ...
    def convert_inter(self):
        ...
        fout.write(
            "\t".join([self.inter_fields[i] for i in range(len(self.inter_fields))])
            + "\n"
        )
        ...
        if self.interaction_type == "timestamp":
            with open(self.inter_file, "r") as f:
                ...
                while True:
                    ...
                    fout.write(
                        str(userid) + "\t" + str(itemid) + "\t" + str(count) + "\n"
                    )
                    line = f.readline()
    ...
    def convert_item(self):
        ...
        fout.write(
            "\t".join([self.item_fields[i] for i in range(len(self.item_fields))])
            + "\n"
        )
...
class ML100KDataset(BaseDataset):
    ...
    def load_inter_data(self):
        return pd.read_csv(
            self.inter_file, delimiter=self.inter_sep, header=None, engine="python"
        )

    def load_item_data(self):
        origin_data = pd.read_csv(
            self.item_file, delimiter=self.item_sep, header=None, engine="python"
        )
    ...
    def load_user_data(self):
        return pd.read_csv(
            self.user_file, delimiter=self.user_sep, header=None, engine="python"
        )
...
class ML1MDataset(BaseDataset):
    ...
    def load_inter_data(self):
        return pd.read_csv(
            self.inter_file, delimiter=self.sep, header=None, engine="python"
        )

    def load_item_data(self):
        origin_data = pd.read_csv(
            self.item_file, delimiter=self.sep, header=None, engine="python"
        )
    ...
    def load_user_data(self):
        return pd.read_csv(
            self.user_file, delimiter=self.sep, header=None, engine="python"
        )
...
class AVAZUDataset(BaseDataset):
    ...
    def convert_inter(self):
        try:
            ...
            with open(self.output_inter_file, "w") as fp, open(
                self.inter_file, encoding="utf-8"
            ) as rp:
...
class TMALLDataset(BaseDataset):
    def __init__(self, input_path, output_path, interaction_type, duplicate_removal):
        ...
        self.interaction_type = interaction_type
        assert self.interaction_type in [
            "click",
            "buy",
        ], "interaction_type must be in [click, buy]"
        ...
        self.output_inter_file = os.path.join(
            self.output_path, self.dataset_name + ".inter"
        )
    ...
    def convert_inter(self):
        try:
            ...
            with open(self.output_inter_file, "w") as fp:
                fp.write(
                    "\t".join(
                        [self.inter_fields[i] for i in range(len(self.inter_fields))]
                    )
                    + "\n"
                )
                if self.duplicate_removal:
...
class NETFLIXDataset(BaseDataset):
    ...
    def load_inter_data(self):
        ...
        raw_path_list = [
            os.path.join(self.input_path, "archive", raw_file)
            for raw_file in raw_file_list
        ]
        lines_list = [
            open(raw_path, encoding="utf-8").read().strip().split("\n")
        ...
        for lines in tqdm(lines_list):
            ...
            while i < len(lines):
                ...
                while i < len(lines) and lines[i][-1] != ":":
                    ...
                    words[-1] = str(
                        int(datetime.strptime(words[-1], time_format).timestamp())
                    )
...
class CRITEODataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # selected feature fields
        self.fields = {0: "index:float", 1: "label:float"}
        for i in range(2, 15):
...
class FOURSQUAREDataset(BaseDataset):
    ...
    def convert_inter(self):
        ...
        data_NYC.iloc[:, -1] = [
            self.utc_to_timestamp(x[-1]) for x in data_NYC.values.tolist()
        ]
        data_TKY.iloc[:, -1] = [
            self.utc_to_timestamp(x[-1]) for x in data_TKY.values.tolist()
        ]
        ...
        if self.merge_repeat == True:
            fout_NYC.write(
                "\t".join([self.fields.get(i) for i in [0, 1, 6, 7, 8]]) + "\n"
            )
            fout_TKY.write(
                "\t".join([self.fields.get(i) for i in [0, 1, 6, 7, 8]]) + "\n"
            )
            ...
            for i in tqdm(range(data_NYC.shape[0])):
                fout_NYC.write(
                    "\t".join([str(data_NYC.iloc[i, j]) for j in [0, 1, 6, 7]]) + "\n"
                )
            for i in tqdm(range(data_TKY.shape[0])):
                fout_TKY.write(
                    "\t".join([str(data_TKY.iloc[i, j]) for j in [0, 1, 6, 7]]) + "\n"
                )
    ...
    def convert_item(self):
        ...
        for data, fout in [(data_NYC, fout_NYC), (data_TKY, fout_TKY)]:
            ...
            for key, value in tqdm(info_dict.items()):
                ...
                key_list.extend(value)
                fout.write(
                    "\t".join([str(key_list[i]) for i in [0, 1, 2, 3, 4]]) + "\n"
                )
...
class DIGINETICADataset(BaseDataset):
    def __init__(self, input_path, output_path, duplicate_removal):
        ...
        self.input_file_prod_cate = os.path.join(
            self.input_path, "product-categories.csv"
        )
        ...
        self.input_file_item_views = os.path.join(
            self.input_path, "train-item-views.csv"
        )
    ...
    def convert_item(self):
        # item part
        self.data_prod_cate = pd.read_csv(
            self.input_file_prod_cate, delimiter=";", header=0, engine="python"
        )
        ...
        fout_prod.write(
            "\t".join([self.fields_item.get(i) for i in self.fields_item.keys()]) + "\n"
        )
        ...
        for j in tqdm(range(lines_count)):
            ...
            fout_prod.write(
                "\t".join([str(line_list[i]) for i in range(len(line_list))]) + "\n"
            )
        fin.close()
        fout_prod.close()
    ...
    def convert_inter(self):
        ...
        fout.write(
            "\t".join([self.fields_inter.get(i) for i in self.fields_inter.keys()])
            + "\n"
        )
            ...
            for key, value in tqdm(sess_clicks.items()):
                ...
                for i in range(1, len(value)):
                    ...
                    if line == inter_pre:
                        ...
                        fout.write(
                            "\t".join(
                                [str(inter_pre[i]) for i in range(len(inter_pre))]
                            )
                            + "\n"
                        )
                        count = 1
                ...
                inter_pre.append(count)
                fout.write(
                    "\t".join([str(inter_pre[i]) for i in range(len(inter_pre))]) + "\n"
                )
        else:
            # divide
            for key, value in tqdm(sess_clicks.items()):
                for i in range(0, len(value)):
                    ...
                    fout.write(
                        "\t".join([str(line[i]) for i in range(len(line))]) + "\n"
                    )
        fout.close()
...
class ANIMEDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # selected feature fields
        self.inter_fields = {0: "user_id:token", 1: "item_id:token", 2: "rating:float"}
        self.item_fields = {
            0: "item_id:token",
            1: "name:token_seq",
            2: "genre:token_seq",
    ...
    def load_item_data(self):
        ...
        processed_data = processed_data.where(
            (processed_data.applymap(lambda x: True if str(x) != "nan" else False)), ""
        )
        processed_data = processed_data.where(
...
class GOWALLADataset(BaseDataset):
    def __init__(self, input_path, output_path, duplicate_removal):
        ...
        self.duplicate_removal = (
            duplicate_removal  # merge repeat interactions if 'repeat' is True
        )
    ...
    def load_inter_data(self):
        if self.duplicate_removal == True:
            ...
            processed_data = origin_data
            processed_data[1] = origin_data[1].apply(
                lambda x: time.mktime(time.strptime(x, "%Y-%m-%dT%H:%M:%SZ"))
            )
        return processed_data
        ...
        with open(self.inter_file, "r") as f:
            ...
            while True:
                if not line:
                    ...
                    break
                line = line.strip().split("\t")
                userid, timestamp, lati, longi, itemid = (
                    line[0],
                    line[1],
                    line[2],
                    line[3],
...
class LFM1bDataset(BaseDataset):
    def __init__(self, input_path, output_path, interaction_type, duplicate_removal):
        ...
        self.item_file = os.path.join(
            self.input_path, "LFM-1b_" + self.interaction_type + ".txt"
        )
        self.user_file = os.path.join(self.input_path, "LFM-1b_users.txt")
        self.user_file_add = os.path.join(
            self.input_path, "LFM-1b_users_additional.txt"
        )
        ...
        if self.interaction_type == "artists":
            self.item_fields = {
                0: self.interaction_type + "_id:token",
                1: "name:token_seq",
            }
        else:
            self.item_fields = {
        ...
        if self.duplicate_removal == True:
            ...
            with open(self.inter_file, "r") as f:
                ...
                while True:
                    ...
                    line = line.strip().split("\t")
                    userid, artistid, albumid, trackid, timestamp = (
                        line[0],
                        line[1],
                        line[2],
                        line[3],
                    ...
                    if self.interaction_type == "artists":
                        ...
                        itemid = trackid
                    fout.write(
                        str(userid) + "\t" + str(itemid) + "\t" + str(timestamp) + "\n"
                    )
                    line = f.readline()
    ...
    def convert_user(self):
        ...
        fout.write(
            "\t".join([self.user_fields[i] for i in range(len(self.user_fields))])
            + "\n"
        )
        ...
        with open(self.user_file, "r") as f1:
            with open(self.user_file_add, "r") as f2:
                ...
                while True:
                    ...
                    fout.write(line1 + "\t")
                    fout.write(
                        "\t".join([line2[i] for i in range(1, len(line2))]) + "\n"
                    )
                    line1 = f1.readline()
                    line2 = f2.readline()
    ...
    def convert_inter(self):
        ...
        with open(self.inter_file, "r") as f:
            ...
            while True:
                ...
                fout.write(
                    "\t".join([str(line[i]) for i in range(len(line) - 1)]) + "\n"
                )
                line = f.readline()
        fout.close()
...
class BOOKCROSSINGDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        self.user_fields = {0: "user_id:token", 1: "location:token_seq", 2: "age:float"}
...
class IPINYOUDataset(BaseDataset):
    def __init__(self, input_path, output_path, interaction_type, duplicate_removal):
        ...
        assert self.interaction_type in [
            "click",
            "view",
        ], "interaction_type must be in [click, view]"
        self.duplicate_removal = duplicate_removal
        ...
        self.output_item_file = os.path.join(
            self.output_path, self.dataset_name + ".item"
        )
        self.output_user_file = os.path.join(
            self.output_path, self.dataset_name + ".user"
        )
        ...
        self.input_view_files = (
            [
                os.path.join(self.input_path, "training1st/imp.%s.txt.bz2" % s)
                for s in days_1st
            ]
            + [
                os.path.join(self.input_path, "training2nd/imp.%s.txt.bz2" % s)
                for s in days_2nd
            ]
            + [
                os.path.join(self.input_path, "training3rd/imp.%s.txt.bz2" % s)
                for s in days_3rd
            ]
        )

        self.input_click_files = (
            [
                os.path.join(self.input_path, "training1st/clk.%s.txt.bz2" % s)
                for s in days_1st
            ]
            + [
                os.path.join(self.input_path, "training2nd/clk.%s.txt.bz2" % s)
                for s in days_2nd
            ]
            + [
                os.path.join(self.input_path, "training3rd/clk.%s.txt.bz2" % s)
                for s in days_3rd
            ]
        )
        ...
        for filepath in self.input_view_files + self.input_click_files:
            ...
            if os.path.exists(new_filepath):
                continue
            with open(new_filepath, "wb") as new_file, bz2.BZ2File(
                filepath, "rb"
            ) as file:
        ...
        self.user_fields = {0: "user_id:token", 1: "user_profile:token_seq"}
        ...
        try:
            ...
            with open(self.output_inter_file, "w") as fp:
                ...
                for k, v in tqdm(inter_data.items()):
                    ...
                    if self.duplicate_removal:
                        fp.write(
                            "\t".join([str(item) for item in list(k) + [v]]) + "\n"
                        )
                    else:
    ...
    def convert_item(self):
        try:
            ...
            with open(self.output_item_file, "w") as fp:
                title = (
                    "\t".join(
                        [self.item_fields[i] for i in range(len(self.item_fields))]
                    )
                    + "\n"
                )
                fp.write(title)
    ...
    def convert_user(self):
        try:
            ...
            with open(self.output_user_file, "w") as fp:
                title = (
                    "\t".join(
                        [self.user_fields[i] for i in range(len(self.user_fields))]
                    )
                    + "\n"
                )
                fp.write(title)
...
class STEAMDataset(BaseDataset):
    ...
    def convert_inter(self):
        ...
        fout.write(
            "\t".join([self.inter_fields.get(i) for i in self.inter_fields.keys()])
            + "\n"
        )
        ...
        for i in tqdm(range(lines_count)):
            ...
            if self.duplicate_removal == False:
                fout.write(
                    "\t".join([str(data_line[i]) for i in range(len(data_line))]) + "\n"
                )
            else:
        ...
        if self.duplicate_removal == True:
            for _, value in tqdm(data_list.items()):
                fout.write("\t".join([str(value[i]) for i in range(len(value))]) + "\n")
    ...
    def convert_item(self):
        fout = open(self.output_item_file, "w")
        fout.write(
            "\t".join([self.item_fields.get(i) for i in self.item_fields.keys()]) + "\n"
        )
...
class PINTERESTDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # selected feature fields
        self.inter_fields = {0: "user_id:token", 1: "item_id:token"}
            ...
            for i in tqdm(range(len(data_test))):
                ...
                for j in range(1, len(data_test[0])):
                    ...
                    if data_test[i][j] != 99:
                        ...
                        fout.write(
                            "\t".join(
                                [str(cur_user_id), str(j - 1), str(data_test[i][j])]
                            )
                            + "\n"
                        )
                cur_user_id = cur_user_id + 1
...
class KDD2010Algebra2006Dataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.train_inter_file = os.path.join(
            self.input_path, "algebra_2006_2007_train.txt"
        )
        self.master_inter_file = os.path.join(
            self.input_path, "algebra_2006_2007_master.txt"
        )
    ...
    def load_inter_data(self):
        train_inter_data = pd.read_csv(
            self.train_inter_file, delimiter=self.sep, engine="python"
        )
        master_inter_data = pd.read_csv(
            self.master_inter_file, delimiter=self.sep, engine="python"
        )
            ...
            if each_field.endswith("Time"):
                ...
                for i in tqdm(range(all_data.shape[0])):
                    if pd.isnull(all_data[each_field][i]):
                        ...
                        continue
                    d = datetime.strptime(
                        str(all_data[each_field][i]), "%Y-%m-%d %H:%M:%S.0"
                    )
    ...
    def convert_inter(self):
        try:
            ...
            self.kdd_convert(
                input_inter_data, self.inter_fields, self.output_inter_file
            )
        except NotImplementedError:
            print("This dataset can't be converted to inter file\n")
...
class KDD2010Algebra2008Dataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.train_inter_file = os.path.join(
            self.input_path, "algebra_2008_2009_train.txt"
        )
            ...
            if each_field.endswith("Time"):
                ...
                for i in tqdm(range(train_inter_data.shape[0])):
                    if pd.isnull(train_inter_data[each_field][i]):
                        ...
                        continue
                    d = datetime.strptime(
                        str(train_inter_data[each_field][i]), "%Y-%m-%d %H:%M:%S.0"
                    )
                    time_str = time.mktime(d.timetuple())
                    this_field.append(time_str)
                time_convert_data[each_field] = pd.Series(this_field)
...
class KDD2010BridgeToAlgebra2006Dataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.train_inter_file = os.path.join(
            self.input_path, "bridge_to_algebra_2006_2007_train.txt"
        )
        self.master_inter_file = os.path.join(
            self.input_path, "bridge_to_algebra_2006_2007_master.txt"
        )
...
class AmazonToolsAndHomeImprovementDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        super(AmazonToolsAndHomeImprovementDataset, self).__init__(
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Tools_and_Home_Improvement.csv"
        )
        self.item_file = os.path.join(
            self.input_path, "meta_Tools_and_Home_Improvement.json"
        )
...
class AmazonInstantVideoDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Amazon_Instant_Video.csv"
        )
...
class AmazonClothingShoesAndJewelryDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        super(AmazonClothingShoesAndJewelryDataset, self).__init__(
            input_path, output_path
        )
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Clothing_Shoes_and_Jewelry.csv"
        )
        self.item_file = os.path.join(
            self.input_path, "meta_Clothing_Shoes_and_Jewelry.json"
        )
...
class AmazonCellPhonesAndAccessoriesDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        super(AmazonCellPhonesAndAccessoriesDataset, self).__init__(
            input_path, output_path
        )
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Cell_Phones_and_Accessories.csv"
        )
        self.item_file = os.path.join(
            self.input_path, "meta_Cell_Phones_and_Accessories.json"
        )
...
class AmazonPatioLawnAndGardenDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Patio_Lawn_and_Garden.csv"
        )
        self.item_file = os.path.join(
            self.input_path, "meta_Patio_Lawn_and_Garden.json"
        )
...
class AmazonGroceryAndGourmetFoodDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        super(AmazonGroceryAndGourmetFoodDataset, self).__init__(
            input_path, output_path
        )
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Grocery_and_Gourmet_Food.csv"
        )
        self.item_file = os.path.join(
            self.input_path, "meta_Grocery_and_Gourmet_Food.json"
        )
...
class AmazonHealthAndPersonalCareDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        super(AmazonHealthAndPersonalCareDataset, self).__init__(
            input_path, output_path
        )
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Health_and_Personal_Care.csv"
        )
        self.item_file = os.path.join(
            self.input_path, "meta_Health_and_Personal_Care.json"
        )
...
class AmazonSportsAndOutdoorsDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Sports_and_Outdoors.csv"
        )
...
class AmazonMusicalInstrumentsDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ratings_Musical_Instruments.csv"
        )
...
class LASTFMDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        self.inter_file2 = os.path.join(
            self.input_path, "user_taggedartists-timestamps.dat"
        )
        self.sep = "\t"
    ...
    def load_item_data(self):
        return pd.read_csv(
            self.item_file, delimiter=self.sep, engine="python", quoting=3
        )
    ...
    def load_inter_data(self):
        ...
        origin_data1 = pd.read_csv(
            self.inter_file1, delimiter=self.sep, engine="python"
        )
        origin_data2 = pd.read_csv(
            self.inter_file2, delimiter=self.sep, engine="python"
        )
        origin_data1["tag_id"] = ""
        index = 0
        ...
        for i in tqdm(range(origin_data2.shape[0])):
            while not (
                origin_data2.iloc[i, 0] == origin_data1.iloc[index, 0]
                and origin_data2.iloc[i, 1] == origin_data1.iloc[index, 1]
            ):
                index += 1
...
class YAHOOMUSICDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # input file
        self.inter_file = os.path.join(
            self.input_path, "ydata-ymusic-user-artist-ratings-v1_0.txt"
        )
        self.item_file = os.path.join(
            self.input_path, "ydata-ymusic-artist-names-v1_0.txt"
        )
        self.sep = "\t"
        ...
        # selected feature fields
        self.inter_fields = {
            0: "anonymous_user_id:token",
            1: "artist_id:token",
            2: "rating:float",
        }
        self.item_fields = {0: "artist_id:token", 1: "name:token"}
    ...
    def convert_item(self):
        ...
        fout.write(
            "\t".join([self.item_fields[column] for column in self.item_fields.keys()])
            + "\n"
        )
...
class YOOCHOOSEDataset(BaseDataset):
    def __init__(self, input_path, output_path, interaction_type, duplicate_removal):
        ...
        if self.interaction_type == "click":
            ...
            self.output_inter_file = os.path.join(
                self.output_path, "yoochoose-clicks.inter"
            )
        elif self.interaction_type == "buy":
            self.inter_file = os.path.join(self.input_path, "yoochoose-buys.dat")
            self.output_inter_file = os.path.join(
                self.output_path, "yoochoose-buys.inter"
            )
    ...
    def convert_inter(self):
        if self.duplicate_removal:
            ...
            for i in tqdm(range(lines_count)):
                ...
                if i == 0:
                    ...
                    current_list.append(
                        int(
                            time.mktime(
                                time.strptime(line_list[1][0:19], "%Y-%m-%dT%H:%M:%S")
                            )
                        )
                    )
                elif (
                    line_list[0] == current_list[0] and line_list[2] == current_list[1]
                ):
                    current_list[2] += 1
                    current_list[3] = int(
                        time.mktime(
                    ...
                    fout.write(
                        "\t".join(
                            [str(current_list[i]) for i in range(len(current_list))]
                        )
                        + "\n"
                    )
                    current_list.clear()
            ...
            fout.write(
                "\t".join([str(current_list[i]) for i in range(len(current_list))])
            )
            fin.close()
            fout.close()
            ...
            for i in tqdm(range(lines_count)):
                ...
                line_list = line.split(",")
                line_list[1] = int(
                    time.mktime(time.strptime(line_list[1][0:19], "%Y-%m-%dT%H:%M:%S"))
                )
                fout.write(
                    "\t".join([str(line_list[i]) for i in range(len(line_list))])
                )
...
class RETAILROCKETDataset(BaseDataset):
    def __init__(self, input_path, output_path, interaction_type, duplicate_removal):
        ...
        assert self.interaction_type in [
            ...
            "addtocart",
            "transaction",
        ], "interaction_type must be in [view, addtocart, transaction]"
        self.duplicate_removal = duplicate_removal
        ...
        if self.interaction_type == "view":
            self.output_inter_file = os.path.join(
                self.output_path, "retailrocket-view.inter"
            )
        elif self.interaction_type == "addtocart":
            self.output_inter_file = os.path.join(
                self.output_path, "retailrocket-addtocart.inter"
            )
        elif self.interaction_type == "transaction":
            self.output_inter_file = os.path.join(
                self.output_path, "retailrocket-transaction.inter"
            )
    ...
    def convert_inter(self):
        if self.duplicate_removal:
            ...
            for i in tqdm(range(lines_count)):
                ...
                line_list[0] = int(time.mktime(time.strptime(line_list[0], "%m/%d/%Y")))
                fout.write(
                    "\t".join([str(line_list[j]) for j in range(len(line_list))]) + "\n"
                )
...
class MINDLargeTrainDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # output file
        self.output_item_file = os.path.join(
            self.output_path, "mind_large_train.item"
        ).replace("\\", "/")
        self.output_inter_file = os.path.join(
            self.output_path, "mind_large_train.inter"
        ).replace("\\", "/")
    ...
    def convert_inter(self):
        ...
        for i in tqdm(range(lines_count)):
            ...
            if "AM" in line_list[2]:
                timestamp = str(
                    int(
                        time.mktime(time.strptime(line_list[2], "%m/%d/%Y %H:%M:%S AM"))
                    )
                )
            else:
                timestamp = str(
                    int(
                        time.mktime(time.strptime(line_list[2], "%m/%d/%Y %H:%M:%S PM"))
                    )
                    + 43200
                )
...
class MINDLargeDevDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # output file
        self.output_item_file = os.path.join(
            self.output_path, "mind_large_dev.item"
        ).replace("\\", "/")
        self.output_inter_file = os.path.join(
            self.output_path, "mind_large_dev.inter"
        ).replace("\\", "/")
...
class MINDSmallTrainDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # output file
        self.output_item_file = os.path.join(
            self.output_path, "mind_small_train.item"
        ).replace("\\", "/")
        self.output_inter_file = os.path.join(
            self.output_path, "mind_small_train.inter"
        ).replace("\\", "/")
...
class MINDSmallDevDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        # output file
        self.output_item_file = os.path.join(
            self.output_path, "mind_small_dev.item"
        ).replace("\\", "/")
        self.output_inter_file = os.path.join(
            self.output_path, "mind_small_dev.inter"
        ).replace("\\", "/")
